...
{ 
	int i, j;        /*Here i is for infix expression and j is for postfix expression and both are initialized to 0*/
	char item;
	char temp;
	...
	while(item != '\0')        /* run loop till end of infix expression */
	{
		...
		{
			temp=pop();   /*Here we popped out the topmost operator and store it in a variable temp*/
			while(is_operator(temp) == 1 && precedence(temp)>= precedence(item))/*temp stores the topmost operator,item stores operator to be pushed*/
			{
				postfix_exp[j] = temp;                  /* so pop all higher precendence operator and */
				j++;
				temp = pop();      /*pop the next element,checks the condition and add them to postfix expresion */
			}
			push(temp);
			/* because just above while loop will terminate we have
			poppped one extra item
		...
		{
			temp = pop();                   /* pop and keep popping until */
			while(temp != '(')                /* '(' encounterd */
			{
				postfix_exp[j] = temp;
				j++;
				temp = pop();
